  ...
  <div style="display: flex; flex-direction: column;">
    <div class="player-wrap" style="position: relative; width: 100%; aspect-ratio: 16/9; background: #000; border-radius: 8px; overflow: hidden; border: 1px solid var(--border);">
      ...
      <div class="player-overlay" id="player-overlay" style="position: absolute; inset: 0; background: rgba(0,0,0,0.85); display: flex; align-items: center; justify-content: center; z-index: 10; color: white;">
        <div class="player-overlay-content stack" style="text-align: center; max-width: 400px; padding: 32px;">
          <p style="font-size: 48px; margin-bottom: 16px;">🎬</p>
          <h2 style="margin-bottom: 12px; color: white;"><?= e($assignment['module_name']) ?></h2>
          <p style="color: #ccc;">Connecting to lighting controller and preparing playback...</p>
          <div id="preflight-status" style="margin: 16px 0; font-size: 14px; opacity: .8; padding: 12px; background: rgba(255,255,255,0.1); border-radius: 6px;"></div>
          <button id="start-btn" class="button primary" style="display:none; font-size: 16px; padding: 12px 24px;">▶ Start Session</button>
          <div id="preflight-error" style="color: #ff8080; margin-top: 16px; display:none; background: rgba(255,0,0,0.1); padding: 12px; border-radius: 6px;"></div>
        </div>
      </div>
...
<script src="<?= e(path('/assets/js/runtime.js')) ?>"></script>
<script>
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  pauseBtn?.addEventListener('click', function() {
    if (!runtime) return;
    if (video.paused) {
      runtime.videoEngine?.resume();
      this.textContent = '⏸ Pause';
    } else {
      runtime.videoEngine?.pause();
